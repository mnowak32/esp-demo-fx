...
Matrix::Matrix(uint8_t *pixels, uint8_t xsize, uint8_t ysize) : xsize(xsize), ysize(ysize) {
  grbPixels = (GrbColor *)pixels;
  
  for (uint16_t i = 0; i < 16; i++) {
    //initialize palette
    palette[i] = GrbColor(RgbColor(HslColor(0.3f, 1.0f, pow(i / 48.0f, 1.8f))));
  }
  drops = new Drop[xsize];
  yLimit = ysize * 2 * 16;
}

void Matrix::newDrops() {
  for(uint8_t x = 0; x < xsize; x++) {
    if (drops[x].y >= yLimit) {
      uint8_t dst = random(32);
